...
function CSPlaySound(action) {
	if(document.embeds[action[1]] != null) {
		if (CSLastSound != null && CSLastSound != action[1])
			document.embeds[CSLastSound].stop();
		CSLastSound = action[1]
		if (window.navigator.userAgent.indexOf("MSIE") > 0)
			{
			if (typeof(document.embeds[CSLastSound].run) != "undefined")
				document.embeds[CSLastSound].run();
			}
		else
			{
			if (typeof(document.embeds[CSLastSound].play) != "undefined")
				document.embeds[CSLastSound].play(true);
			}
	}
	else
	{
	alert ("The current Plug-In can not be controlled by JavaScript. Please try using LiveAudio or a compatible Plug-In!");
	}
}
...
function CSStopSound (action) {
	if (document.embeds[action[1]] !=null) {
		if( typeof(document.embeds[action[1]].stop) != "undefined" )
			document.embeds[action[1]].stop();
	}
}
